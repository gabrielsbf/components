    ...
    def collect_filtered_post_links(self, start_date :  datetime, end_date : datetime)-> list[dict]:
        ...
        while continue_collecting:
            ...
            for i in range(total_posts):
                ...
                datetime_element = post.locator('a:has(time)').first
                datetime_element.wait_for(state="visible", timeout=5000)

                datetime_str = datetime_element.locator("time").get_attribute("datetime")
                post_datetime = datetime.strptime(datetime_str, "%Y-%m-%dT%H:%M:%S.%fZ")

                post_url = datetime_element.get_attribute("href")
                ...
                print(f"Data do post: {post_datetime}")
                print(f"Link encontrado: {post_url}")
                print(f"Descrição do post: {post_decription}")
                print(f"Métricas do post: {engagement_summary_str}")
                print(f"Métricas convertidas: {post_metrics}")
                ...
                if start_date <= post_datetime <= end_date:
                    ...
                    print(f"Link adicionado: {post_url}")
        print("Links filtrados:", filtered_posts)
        print(f"Total de links filtrados: {len(filtered_posts)}")
