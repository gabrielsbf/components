...
class Twitter_Automation(PlayEssencial):
    ...
    def get_href(self, since: str, until: str):
        ...
        while last_date >= since:
            ...
            access_last_date = last_post.locator('//a[@class="css-146c3p1 r-bcqeeo r-1ttztb7 r-qvutc0 r-37j5jr r-a023e6 r-rjixqe r-16dba41 r-xoduu5 r-1q142lx r-1w6e6rj r-9aw3ui r-3s2u2q r-1loqt21"]')
            last_datetime_str = access_last_date.locator("time").get_attribute("datetime")
            if last_datetime_str:
                last_date = datetime.strptime(last_datetime_str, "%Y-%m-%dT%H:%M:%S.000Z")
                end_loop = last_date < since
            for i in range(count):
                post = posts.nth(i)
                access_post_data = post.locator('//a[@class="css-146c3p1 r-bcqeeo r-1ttztb7 r-qvutc0 r-37j5jr r-a023e6 r-rjixqe r-16dba41 r-xoduu5 r-1q142lx r-1w6e6rj r-9aw3ui r-3s2u2q r-1loqt21"]')
                post_date = access_post_data.locator("time").get_attribute("datetime")
                post_date = datetime.strptime(post_date, "%Y-%m-%dT%H:%M:%S.000Z")
                ...
                access_link = access_post_data.get_attribute("href")
                get_href = ''.join(["https://www.x.com", access_link])
                if get_href in list(links_filtrados.keys()):
                    print("ja existe")
                    continue
                else:
                    if since <= post_date <= until:
                        links_filtrados.update({get_href : {'Descrição' : post_descs, 'Data' : post_date.strftime("%d/%m/%Y %H:%M:%S")}})
